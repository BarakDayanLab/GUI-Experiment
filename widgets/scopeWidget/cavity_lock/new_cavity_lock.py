...
from services.lorentzian_fit.resonance_fit import ResonanceFit
...
class Cavity_lock_GUI(Scope_GUI):
    ...
    def __init__(self, Parent=None, ui=None, debugging=False, simulation=True):
        ...
        self.resonance_fit = ResonanceFit()
    ...
    def update_scope(self, data, parameters):
        ...
        rubidium_lines, transmission_spectrum = data[0], data[1]

        if self.checkBox_Rb_lines.isChecked():
            self.resonance_fit.calibrate_freq_axis(rubidium_lines)
        if self.checkBox_Cavity_transm.isChecked():
            self.resonance_fit.fit_transmission_spectrum(transmission_spectrum)
        ...
        rubidium_peaks = self.resonance_fit.get_rubidium_peaks()  # all the peaks as coordinates
        x0_coords = self.resonance_fit.get_x0_coords()  # all the peaks as coordinates
        if self.selectedPeaksXY is not None and type(self.selectedPeaksXY) == list:# and len(self.selectedPeaksXY) == 2 and type(self.selectedPeaksXY[0]) == np.ndarray and type(self.selectedPeaksXY[1]) == np.ndarray:
            self.updateSelectedPeak([rubidium_peaks, x0_coords])
        ...
        avg_data = self.resonance_fit.get_current_data()
        x_scatter, y_scatter = np.vstack((rubidium_peaks, x0_coords)).T
        x_ticks = self.resonance_fit.freq_axis
        self.widgetPlot.plot_Scope(x_axis, avg_data, autoscale=self.checkBox_plotAutoscale.isChecked(), redraw=redraw, labels=labels, x_ticks=x_ticks, y_ticks=y_ticks,
                                   aux_plotting_func=self.widgetPlot.plot_Scatter, scatter_y_data=y_scatter,
                                   scatter_x_data=x_scatter, mark_peak=self.selectedPeaksXY, text_box=text_box_string)
