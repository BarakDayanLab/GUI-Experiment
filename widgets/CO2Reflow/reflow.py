...
class Keithley3390Controller():
    # 1421680
    __instID = 'Keithley Instruments Inc.,3390,1421680,1.02-0B1-03-02-02'
    ...
    def recallStateFromMemory(self, state=3):
        self.AWG.write('*RCL %d' % int(state))
...
class ReflowController(QWidget):
    ...
    def detectCircles(self, img=None, brightness_threshold=130, minDist=100, param1=50, param2=10, minRadius=20,
                      maxRadius=100, showDetectedCircles=False):
        if img is None: img = self.grabImage()

        brightness_threshold, minDist = self.spinBox_brightnessThreshold.value(), self.spinBox_minDist.value()
        param1, param2 = self.spinBox_P1.value(), self.spinBox_P2.value()
        minRadius, maxRadius = self.spinBox_minRadius.value(), self.spinBox_maxRadius.value()
        ...
        blackAndWhite = cv2.inRange(img, brightness_threshold,
                                    255)  # Create a mask - this turns the photo to B&W (no grays!)
        blurred = cv2.medianBlur(blackAndWhite,
                                 25)  # Blur everything. This is important for circle detection and noise filtering
    ...
    def detectAndMoveCircleToTarget(self, target, showDetectedCircles=False):
        ...
        output_limits = (
        self.__distanceToroidsInRow * (-0.4), self.__distanceToroidsInRow * 0.4)  # don't move more than limit, in um
    ...
    def moveToNextToroid(self, moveDown=False):
        ...
        self.checkBox_findNextToroid.setChecked(True)
        yDirection = (-1) if moveDown else 1
        previousToroidStillInFrame = True
        newToroidInFrame = False
        while (previousToroidStillInFrame or not newToroidInFrame) and self.checkBox_findNextToroid.checkState():
        ...
        self.spinBox_toroidIndex.setValue(self.spinBox_toroidIndex.value() + 1)
        self.checkBox_findNextToroid.setChecked(False)

    def reflowEntireRow(self, target=(640, 320), discsLeftInRow=18, firstToroidNumber=1, rowNumber=1, moveDown=False):
        while self.spinBox_toroidIndex.value() < self.__toroidsInRow:
