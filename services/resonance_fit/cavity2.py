...
class Cavity:
    def __init__(self, main_parameter: str, fit_parameters: list, bounds):
        ...
        self.base_fit_parameters = ["x_0", "m", "b"]
        ...
        self.base_bounds = [(-np.inf, -np.inf, -np.inf), (np.inf, np.inf, np.inf)]
...
class CavityKex(Cavity):
    ...
    def transmission_spectrum_func(self, detunings):
        ...
        return self.linear_factor(detunings) * (1 + utils.transmission_spectrum(detunings, self.k_ex, self.k_i, self.h))

    def reflection_spectrum_func(self, detunings):
        detunings = detunings - self.x_0
        return self.linear_factor(detunings) * (1 + utils.reflection_spectrum(detunings, self.k_ex, self.k_i, self.h))
...
class CavityFwhm(Cavity):
    ...
    def transmission_spectrum_func(self, detunings):
        detunings = detunings - self.x_0
        return self.linear_factor(detunings) * (1 + utils.lorentzian_spectrum(detunings, self.fwhm, self.amp))
